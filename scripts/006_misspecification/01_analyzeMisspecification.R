...
parm.table$p.m[1:6] = 0.1
parm.table$p.m[7:12] = 0.2
parm.table$p.m[13:18] = 0.3
...
for(i in 1:dim(panels.pm)[1]){
  ...
  # get data for panel
  df.tmp <- df.pm %>% dplyr::filter(p.m==panels.pm$p.m[i] & p.g==panels.pm$p.g[i])
  
  if(focalParam=="p.m"){yax = .02}else{yax = 0}
  plot(NA,
       xlim=c(0,32), ylim = c(-.2, .2),
       ylab='',
       xlab="",
       yaxt='n',
       xaxt='n')
...
# get data for panel
df.tmp <- df %>% dplyr::filter(p.m==panels.pg$p.m & p.g==panels.pg$p.g)
...
plot(NA,
     ...
     yaxt='n',
     xaxt='n')
axis(1, at = seq(0 , 30, by = 10), las=1, cex = pt7,labels=F)
...
for(i in 1:dim(panels.pm)[1]){
  ...
  if(i==1){
    mtext(side=2, line = 2.5, "Coverage", cex = pt9)
  }
...
for(i in 1:dim(panels.pm)[1]){
  ...
  plot(NA,
       xlim=c(0,32), ylim = c(0, 0.15),
       ylab='',
       xlab="",
       yaxt='n',
       xaxt='n')
  axis(1, at = seq(0 , 30, by = 10), las=1, cex = pt7, labels=T)
  if(i==1){ axis(2, at = seq(0, .15, by = .025), labels = F);
    axis(2, at = seq(0, .15, by = .05), las=1, cex = pt7) }
  
  abline(h = 0, col='gray50')
  box()
  
  points(x= df.tmp$n.bags+ c(-0.75,0.75), y = df.tmp$rmse ,
         pch=ifelse(df.tmp$experiment=="bags",21,21),
         bg=ifelse(df.tmp$experiment=="bags","black","white"))
...
mtext(side=1, line = 1, "Number of samples (bags or plots)", cex = pt9,  outer = TRUE)
