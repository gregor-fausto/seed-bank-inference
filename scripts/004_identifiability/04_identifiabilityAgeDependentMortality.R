...
for(i in 1:3){
  # get filenames
  fileNamesBags=list.files(paste0(primaryDirectory,"outputs/002_statisticalModelFitting/01_identifiability/posteriors-NpVmCg-",i,"/seedBagBurial/"))
  ...
  fits.Np.bags = paste0(primaryDirectory,"outputs/002_statisticalModelFitting/01_identifiability/posteriors-NpVmCg-",i,"/seedBagBurial/",fileNamesBags[index.bags])
...
for(i in 1:3){
  # get filenames
  fileNamesAddition=list.files(paste0(primaryDirectory,"outputs/002_statisticalModelFitting/01_identifiability/posteriors-NpVmCg-",i,"/seedAddition/"))
  ...
  # list of full file paths
  fits.Np.add = paste0(primaryDirectory,"outputs/002_statisticalModelFitting/01_identifiability/posteriors-NpVmCg-",i,"/seedAddition/",fileNamesAddition[index.add])
...
tiff(filename=paste0(primaryDirectory,"products/figures/prior-posterior-overlap-variable-mortality.tif"),
...
tiff(filename=paste0(primaryDirectory,"products/figures/identifiability-joint-variable-1.tif"),
...
plot(post.bags[,2],post.bags[,1],pch=16,ylab="",xlab="",cex=.5,
     xlim=c(.09,.116),ylim=c(.09,.116),tick=TRUE,
     axes=FALSE)
...
plot(post.add[,2],post.add[,1],pch=21,cex=.5,xlab="",ylab="",
     xlim=c(0,1),ylim=c(0,1),tick=TRUE,
     axes=FALSE)
...
tiff(filename=paste0(primaryDirectory,"products/figures/identifiability-joint-variable-2.tif"),
     height=3,width=2,units="in",res=300,compression="lzw",pointsize=12)
...
plot(post.bags[,2],post.bags[,1],pch=16,cex=.4,ylab="",xlab="",
     xlim=c(.07,.12),ylim=c(.07,.12),
     xaxt='n',yaxt='n')

axis(2,cex.axis=pt6,tick=TRUE,outer=FALSE,at=seq(.07,.12,by=.01))
...
tiff(filename=paste0(primaryDirectory,"products/figures/identifiability-joint-variable-3.tif"),
     height=3,width=2,units="in",res=300,compression="lzw",pointsize=12)
# age 3
...
axis(2,cex.axis=pt6,tick=TRUE,outer=FALSE,at=seq(.07,.14,by=.02))
